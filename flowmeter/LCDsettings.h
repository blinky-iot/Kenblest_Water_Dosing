...
public:
  ...
  void startScreen();
  void displayStatus(bool manualMode, bool meterRunning, int target, float total, bool modbusOK,float batchAmount,bool update = true);
  void setSettings(int target);
  void batchUpdater(float batch, float total);
  void manualDisplayStatus(bool manualMode, bool meterRunning, float total, bool modbusOK,float batchAmount);
  void LCDsetup();
};
